...
function jerroCircleAnimation(){
	...
	function newState(){ //go to the new state function
		$('#'+randomID+' .back-content').transition({ opacity: 1 });// show back-content
		$('#'+randomID+' .front-content').transition({ opacity: 0 });// hide front-content
		$('#'+randomID).addClass(' now-used');
		//console.log(randomID+' = 1');
	}

	function baseState(){ //return to the base state function
		$('#'+randomID+' .back-content').transition({ opacity: 0 });// hide back-content
		$('#'+randomID+' .front-content').transition({ opacity: 1 });// show fron-content
...
$( '.jerro-circle, .services-box, .posts-grid li' ).click(

	function () {
		ohSnap('Soon I can do something...', 'default');
	}
